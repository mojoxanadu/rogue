#!/usr/bin/env python3
"""
Generate an HTML asset selector for roguelike sprites with all game items and improved UI.
"""
import os
import json
import re
...
def parse_item_def():
    ...
    for key in matches:
        # Try to extract name from the value object
        # Look for "name": "..."
...
def generate_html():
    ...
    for key, config in sprite_defs.items():
        if 'detailed' in config or key in ['slime', 'skeleton', 'bat', 'ghost', 'robot', 'dragon', 'snake', 'troll', 'medusa', 'assassin', 'killer_rabbit', 'chipmunk', 'duck', 'shark', 'wet_rat', 'duck_hunt_dog'] or key == 'player':
    ...
    # 4. Items from sprite_defs (emoji items)
    item_list = []
    emoji_keys = ['💣🌟', '🧪', '🗡️', '🛡️', '💰']
    for key, config in sprite_defs.items():
        if key in emoji_keys:
            item_list.append((key, config, 'sprite'))
    categories['Items (Sprite)'] = item_list
    ...
    # 6. Monsters from MONSTER_DEF not in sprite_defs
    extra_monsters = []
    ...
    html.append('<html><head><meta charset="UTF-8"><title>Roguelike Asset Selector v4</title>')
    ...
    html.append('th, td { border: 1px solid #444; padding: 12px; text-align: center; vertical-align: middle; }')
    ...
    html.append('.col-key { width: 10%; font-family: monospace; color: #f1c40f; }')
    html.append('.col-name { width: 20%; text-align: left; }')
    html.append('.col-current { width: 10%; }')
    html.append('.col-lpc { width: 25%; }')
    html.append('.col-dcss { width: 25%; }')
    html.append('.col-selected { width: 10%; color: #2ecc71; font-size: 0.9em; }')
    ...
    html.append('.suggestions { display: flex; flex-wrap: wrap; gap: 4px; justify-content: center; min-height: 36px; }')
    ...
    html.append('<h1>🎮 Roguelike Asset Selector v4</h1>')
    ...
    for cat_name, cat_items in categories.items():
        ...
        for key, config, source_type in cat_items:
            # Key column
            key_display = key if len(key) <= 10 else key[:8] + '...'
            ...
            if any(ord(c) > 127 for c in key):
                ...
                html.append(f'<td class="col-current"><div class="thumb color-thumb" style="background: {color};" data-source="color" data-path="{color}" title="Current color: {color}"></div></td>')
...
if __name__ == '__main__':
    html = generate_html()
    with open('asset_selector_v4.html', 'w') as f:
        f.write(html)
    print('Generated asset_selector_v4.html')